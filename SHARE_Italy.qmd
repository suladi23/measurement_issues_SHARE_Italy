...
```{r}
median_hui <- median(Italy$HUI, na.rm = TRUE)
mean_hui <- mean(Italy$HUI, na.rm = TRUE)

# Print the results
print(paste("Median HUI:", round(median_hui, 2)))
print(paste("Mean HUI:", round(mean_hui, 2)))
```


Since the HUI is difficult to measure correctly, it is better to have a look into inequality in subgroups by only considering the perceived health status as it is, in subjective terms, then look at it in more objective terms using the chronic diseases.
